   ...
   public function tablaDetalle(Request $request){
    if(auth()->user()->type==env('USER_COMUN')){
        ...
        if($request->rancho!='TOTAL'){
            $datos = DB::select("SELECT * from tablas WHERE CODIGO!='0' and PRODUCTO='".$request->producto."' and RANCHO='".$request->rancho."' ".$anio);
        }
        //Si no, muestra filtrado por PRODUCTO, RANCHO y AÑO
        else{
            $datos = DB::select("SELECT * from tablas WHERE CODIGO!='0' and PRODUCTO='".$request->producto."' ".$anio);
        }
            //almacenamos los valores de los encabezados de la tabla 
            $headers = array();
