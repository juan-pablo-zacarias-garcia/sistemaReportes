...
<hr>
<table id="tablaAgroquimicosPromedioXHa" class="table table-bordered table-condensed table-striped col-md-8">
    <thead>
        <tr>
        ...
        <tr>
            ...
            <td>
                <form method="POST" action="{{ route('detallesTablas') }}">
                    ...
                    <input class="alert-link" type="submit"
                        value="{{$dato==0?'':(is_numeric($dato) && key($datos2[0])!='Hectareas'?'$'.number_format($dato, 2):$dato)}}" />
                </form>
            </td>
            @else
            <td>{{$dato==0?'':(is_numeric($dato)?'$'.number_format($dato, 2):$dato)}}</td>
